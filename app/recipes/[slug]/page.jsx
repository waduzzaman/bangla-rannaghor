...
import { initialRecipes } from "@/data/recipes"; // Ensure this path is correct
...
export async function generateMetadata({ params }) {
  ...
  if (!recipe) notFound();
  return {
    title: `${recipe.title} Recipe - BanglaRannaghor`,
    ...
    <div className="max-w-4xl mx-auto bg-white p-6 sm:p-10 rounded-xl shadow-xl">
      ...
      <div className="flex flex-wrap gap-4 text-sm font-semibold text-gray-700 mb-8 border-b pb-4">
        ...
        <span className="bg-gray-100 px-3 py-1 rounded-full">
          <span role="img" aria-label="prep time">
            ⏱
          </span>{" "}
          Prep: {recipe.prepTime}
        </span>
        <span className="bg-gray-100 px-3 py-1 rounded-full">
          <span role="img" aria-label="cook time">
            🔥
          </span>{" "}
      ...
      <div className="w-full h-96 mb-8 rounded-lg shadow-md overflow-hidden">
        {recipe.youtubeId ? (
          <iframe
          ...
          <Image
            width={800}
            height={400}
            src="https://placehold.co/800x400/2dd4bf/000000?text=Recipe+Photo"
            alt={recipe.title}
      ...
      {/* Ingredients */}
      <h2 className="text-3xl font-bold text-gray-800 mt-8 mb-4 border-b-2 border-amber-500 pb-1">
        Ingredients
      </h2>
      ...
      {/* Instructions */}
      <h2 className="text-3xl font-bold text-gray-800 mb-4 border-b-2 border-amber-500 pb-1">
        Instructions
      </h2>
      ...
      <div className="mt-10 text-center">
        <Link
          href="/recipes"
          className="text-teal-600 hover:text-teal-800 font-semibold text-lg transition duration-200"
        >
          ← Back to All Recipes
        </Link>
      </div>
